...
`advSearch.fileGlob` 설정에서 검색에 포함할 파일 확장자를 Glob 패턴으로 지정할 수 있습니다. 기본값은 `*.{c,cpp,h,hpp,js,ts}`입니다. 예를 들어 `*.{py,rs}` 로 변경하면 파이썬과 러스트 파일만 검색합니다.

`advSearch.maxResults` 설정으로 한 번에 표시할 검색 결과 수를 지정할 수 있습니다. 기본값은 `1000`이며, 결과가 이 값을 초과하면 패널 하단의 **더 보기** 버튼으로 다음 결과를 불러올 수 있습니다.


## 기술적 특징

- **고성능**: 비동기 방식으로 여러 파일을 동시에 검색
